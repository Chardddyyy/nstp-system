...
export function StudentAttendanceMatrixModal({ 
  ...
  useEffect(() => {
    ...
    // Only show spinner if there is no student data available
    const hasInitial = (propStudents && propStudents.length > 0) || 
      (authStudents && authStudents.length > 0) || 
      Boolean(localStorage.getItem('nstp_cached_students'));
    loadData(!hasInitial);

    const handleUpdate = () => {
      // Re-fetch silently in background without showing full-page spinner
      loadData(false);
    };
