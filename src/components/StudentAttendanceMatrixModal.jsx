...
import { X, Search, FileSpreadsheet, UserX, CheckCircle, Clock, AlertTriangle, Users, Download, Edit3, Trash2, Check, ShieldCheck, HelpCircle, RefreshCw, RotateCcw } from 'lucide-react';
...
export function StudentAttendanceMatrixModal({ 
  ...
  const handleClearAllAttendance = async () => {
    try {
      setClearing(true);
      await attendanceAPI.clearAll(selectedDept || 'All');
      ...
      showToast?.('All attendance records have been cleared! Attendance is reset to Day 1.', 'success');
      try {
        window.dispatchEvent(new CustomEvent('nstp_attendance_updated'));
      } catch (_) {}
    } catch (err) {
      console.error('Failed to clear attendance:', err);
      showToast?.('Failed to clear attendance records.', 'error');
    } finally {
      setClearing(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/80 backdrop-blur-md flex items-center justify-center p-1.5 sm:p-4 select-none">
      <div className="bg-white rounded-2xl sm:rounded-3xl max-w-6xl w-full max-h-[96vh] sm:max-h-[92vh] flex flex-col shadow-2xl border border-slate-200 overflow-hidden animate-slide-up relative">
        ...
        <div className="p-2 sm:p-4 bg-white border-b border-slate-200 flex flex-wrap items-center justify-between gap-2 sm:gap-3 text-xs">
          ...
          {/* Export Master PDF & Excel, Reset to Day 1, and Refresh */}
          <div className="flex items-center gap-1.5 flex-wrap">
            <button
              type="button"
              onClick={() => setShowClearConfirm(true)}
              disabled={loading || clearing}
              className="px-2.5 sm:px-3 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold text-[10.5px] sm:text-xs rounded-md sm:rounded-lg shadow-xs active:scale-95 transition-all flex items-center gap-1 cursor-pointer disabled:opacity-50 border border-rose-200"
              title="Clear all student attendance records and reset to Day 1"
            >
              <RotateCcw className="w-3.5 h-3.5 text-rose-600" />
              <span className="hidden xs:inline">Reset to Day 1</span>
            </button>
            <button
              type="button"
          ...
          <div className="fixed inset-0 z-60 bg-slate-900/80 backdrop-blur-xs flex items-center justify-center p-3">
            <div className="bg-white rounded-3xl max-w-md w-full p-5 sm:p-6 shadow-2xl border border-rose-100 space-y-4 animate-scale-in">
              ...
              <div className="text-center space-y-1.5">
                <h4 className="text-base font-black text-slate-900">Clear All Attendance Records?</h4>
                <p className="text-xs text-slate-500 font-medium">
                  Buburahin nito ang lahat ng attendance logs ng mga estudyante at ibabalik ang attendance tracking sa <b>Day 1</b>.
                </p>
              </div>
              <div className="pt-2 flex items-center gap-2">
                ...
                >
                  ...
                  <span>{clearing ? 'Clearing...' : 'Yes, Reset to Day 1'}</span>
                </button>
              </div>
            </div>
          </div>
